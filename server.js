...
io.on('connection', (socket) => {
    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        /*Chekc that the data coming from the client is good*/
    ...
    socket.on('game_start', (payload) => {
        ...
        io.in(room).allSockets().then((sockets) => {
            /* Engaged player isn't join the room */
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.has(requested_user)) {
                response = {
                    result: 'fail',
...
function check_line_match(color, dr, dc, r, c, board){
    ...
    /* Check to make sure we aren't going to walk off the baord */
    if ((r + dr < 0) || (r + dr > 7)){
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)){
...
function adjacent_support(who, dr, dc, r, c, board){
    ...
    /* Check to make sure that therer is space for a matching color to capture tokens */
